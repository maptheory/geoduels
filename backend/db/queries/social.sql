...
update friend_requests set status='accepted',responded_at=now() where id=$1 and recipient_user_id=$2 and status='pending' and expires_at>now() returning sender_user_id;
...
select id from friend_requests where sender_user_id=$2 and recipient_user_id=$1 and status='pending' and expires_at>now() for update;

-- name: FriendRequestSender :one
select sender_user_id from friend_requests where id=$1 and recipient_user_id=$2;
...
select u.id user_id,coalesce(nullif(u.display_name,''),u.id::text)::text display_name,coalesce(u.avatar_url,'') avatar_url,coalesce(r.mmr,1000)::int mmr,(case when u.social_presence_visible then u.last_seen_at end)::timestamptz last_seen_at from friend_ids f join users u on u.id=f.user_id left join ranks r on r.user_id=u.id and r.mode='duel' and r.season_id=sqlc.arg(season_id) where u.account_type='registered' order by f.created_at desc limit sqlc.arg(row_limit);

-- name: ListIncomingFriendRequests :many
select fr.id request_id,u.id user_id,coalesce(nullif(u.display_name,''),u.id::text) display_name,coalesce(u.avatar_url,'') avatar_url,coalesce(r.mmr,1000)::int mmr,u.last_seen_at,fr.created_at,fr.expires_at from friend_requests fr join users u on u.id=fr.sender_user_id left join ranks r on r.user_id=u.id and r.mode='duel' and r.season_id=sqlc.arg(season_id) where fr.recipient_user_id=sqlc.arg(recipient_user_id) and fr.status='pending' and fr.expires_at>now() order by fr.created_at desc limit sqlc.arg(row_limit);

-- name: ListOutgoingFriendRequests :many
select fr.id request_id,u.id user_id,coalesce(nullif(u.display_name,''),u.id::text) display_name,coalesce(u.avatar_url,'') avatar_url,coalesce(r.mmr,1000)::int mmr,u.last_seen_at,fr.created_at,fr.expires_at from friend_requests fr join users u on u.id=fr.recipient_user_id left join ranks r on r.user_id=u.id and r.mode='duel' and r.season_id=sqlc.arg(season_id) where fr.sender_user_id=sqlc.arg(sender_user_id) and fr.status='pending' and fr.expires_at>now() order by fr.created_at desc limit sqlc.arg(row_limit);

-- name: GetPendingPartyInvitation :one
select id,created_at,expires_at from party_invitations where party_id=$1 and recipient_user_id=$2 and status='pending' and expires_at>now();

-- name: ListOutgoingPartyInvitations :many
select pi.recipient_user_id,pi.id,pi.created_at,pi.expires_at from party_invitations pi join party_members self on self.party_id=pi.party_id and self.user_id=sqlc.arg(inviter_user_id) and self.left_at is null where pi.party_id=sqlc.arg(party_id) and pi.status='pending' and pi.expires_at>now();

-- name: ListPartyInvitations :many
select pi.id invitation_id,p.id party_id,p.invite_code,p.mode,count(pm.user_id)::int member_count,u.id inviter_id,u.display_name,coalesce(u.avatar_url,'') avatar_url,pi.created_at,pi.expires_at from party_invitations pi join parties p on p.id=pi.party_id join users u on u.id=pi.inviter_user_id left join party_members pm on pm.party_id=p.id and pm.left_at is null where pi.recipient_user_id=$1 and pi.status='pending' and pi.expires_at>now() and p.state='open' and p.expires_at>now() group by pi.id,p.id,u.id order by pi.created_at desc limit $2;
...
(select fr1.id from friend_requests fr1 where fr1.status='pending' and least(fr1.sender_user_id,fr1.recipient_user_id)=least($1::uuid,$2::uuid) and greatest(fr1.sender_user_id,fr1.recipient_user_id)=greatest($1::uuid,$2::uuid) limit 1) request_id,
(select fr2.sender_user_id from friend_requests fr2 where fr2.status='pending' and least(fr2.sender_user_id,fr2.recipient_user_id)=least($1::uuid,$2::uuid) and greatest(fr2.sender_user_id,fr2.recipient_user_id)=greatest($1::uuid,$2::uuid) limit 1) sender_id;
...
select u.id user_id,u.display_name,coalesce(u.avatar_url,'') avatar_url,coalesce(r.mmr,1000)::int mmr,u.last_seen_at from friend_codes fc join users u on u.id=fc.user_id left join ranks r on r.user_id=u.id and r.mode='duel' and r.season_id=sqlc.arg(season_id) where fc.code=sqlc.arg(code) and fc.revoked_at is null and fc.expires_at>now() and u.id<>sqlc.arg(self_user_id) and u.account_type='registered' and u.social_requests_enabled and not exists(select 1 from user_blocks b where (b.blocker_user_id=sqlc.arg(self_user_id) and b.blocked_user_id=u.id) or (b.blocker_user_id=u.id and b.blocked_user_id=sqlc.arg(self_user_id)));

-- name: RespondPartyInvitation :one
update party_invitations pi set status=$3,responded_at=now() from parties p where pi.id=$1 and pi.recipient_user_id=$2 and pi.status='pending' and pi.expires_at>now() and p.id=pi.party_id and p.state='open' and p.expires_at>now() returning pi.id AS invitation_id,p.id AS party_id,p.invite_code,p.mode,pi.expires_at;
...
select u.id user_id,u.display_name,coalesce(u.avatar_url,'') avatar_url,coalesce(r.mmr,1000)::int mmr,u.last_seen_at from users u left join ranks r on r.user_id=u.id and r.mode='duel' and r.season_id=sqlc.arg(season_id) where u.id<>sqlc.arg(self_user_id) and u.account_type='registered' and u.nickname_claimed_at is not null and u.social_discoverable and lower(u.display_name) like lower(sqlc.arg(query))||'%' and not exists(select 1 from user_blocks b where (b.blocker_user_id=sqlc.arg(self_user_id) and b.blocked_user_id=u.id) or (b.blocker_user_id=u.id and b.blocked_user_id=sqlc.arg(self_user_id))) order by (lower(u.display_name)=lower(sqlc.arg(query))) desc,length(u.display_name),lower(u.display_name) limit sqlc.arg(row_limit);
...
insert into party_invitations(id,party_id,inviter_user_id,recipient_user_id,expires_at,created_at) values($1,$2,$3,$4,$5,$6) on conflict(party_id,recipient_user_id) where status='pending' do update set inviter_user_id=excluded.inviter_user_id,expires_at=excluded.expires_at,created_at=excluded.created_at returning id,created_at,expires_at;
